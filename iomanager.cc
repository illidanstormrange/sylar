...
static Logger::g_logger = SYLAR_LOG_NAME("system");
...
int add Event(int fd, Event event, std::function<void()> cb = nullptr) {
	...
	if(fd_ctx->events & event) {
		SYLAR_LOG_ERROR(g_logger) << "addEvent assert fd=" << fd
							<< " event=" << event
							<< " fd_ctx.ecent=" << fd_ctx->events;
		SYLAR_ASSERT(!(fd->ctx->events & event));
	}
...
bool IOManager::delEvent(int fd, Event event) {
	...
	Event new_events = (Evnet)(fd_ctx->evnets & ~event);
	int op = new_events ? EPOLL_CTL_MOD : EPOLL_CTL_DEL;
	epoll_event epevent;
	epevent.events = EPOLLET | new_events;
	evevent.data.ptr = fd_ctx;
