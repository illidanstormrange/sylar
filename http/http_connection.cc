...
HttpResponse::ptr HttpConnection::recvReponse() {
	...
	if(client_parser.chunked) {
		...
		do {
			do {
				int rt = Streamread(data + len, buff_size - len);
				if(rt <= 0) {
					close();
					return nullptr;
				}
			...
			if(client_parser.content_len <= len) {
				...
				while(left > 0) {
					int rt = Streamread(data, left > (int)buff_size ? (int)buff_size : left);
					if(rt <= 0 ) {
						close();
						return nullptr;
					}
...
HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req
	...
	if(rt == 0) {
		return std::make_shared<HttpResult>((int)HttpResult::Error::SEND_CLOSE_BY_PEER
				, nullptr, "send requset closed by peer: " + addr->toString());
	}
	if(rt < 0) {
		return std::make_shared<HttpResult>((int)HttpResult::Error::SEND_SCOET_ERROR
				, nullptr, "send requset socket error errno=: " + std::to_string(errno)
				+ " errstr = " + std::string(strerror(errno)));
	}
...
HttpConnectionPool::HttpConnectionPool(const std::string& host
						...
						,uint32_t max_requset)
	:m_host(host)
	,m_vhost(vhost)
	,m_port(port)
	,m_maxSize(max_size)
	,m_maxAliveTime(max_alive_time)
	,m_maxRequest(max_requset){
}
...
HttpResult::ptr HttpConnectionPool::doRequest(HttpRequest::ptr req
	...
	if(!sock) {
		return std::make_shared<HttpResult>((int)HttpResult::Error::POOL_INVALD_CONNECTION
				, nullptr, "pool host: " + m_host + "port:" + std::to_string(m_port));
	...
	if(rt == 0) {
		return std::make_shared<HttpResult>((int)HttpResult::Error::SEND_CLOSE_BY_PEER
				, nullptr, "send requset closed by peer: " + sock->getRemoteAddress()->toString());
	}
	if(rt < 0) {
		return std::make_shared<HttpResult>((int)HttpResult::Error::SEND_SCOET_ERROR
				, nullptr, "send requset socket error errno=: " + std::to_string(errno)
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
	++ptr->m_requset;
	if(!ptr->isConnected()
			|| (ptr->m_requset >= pool->m_maxRequest)
