...
std::pair<HttpResult::ptr, WSConnection::ptr> WSConnection::Create(Uri::ptr uri
    ...
    if(!sock->connect(addr)) {
        return std::make_pair(std::make_shared<HttpResult>((int)HttpResult::Error::CONNECTION_FAIL
   ...
   int rt = conn->sendRequest(req);
    ...
    if(rt < 0) {
        return std::make_pair(std::make_shared<HttpResult>((int)HttpResult::Error::SEND_SCOET_ERROR
