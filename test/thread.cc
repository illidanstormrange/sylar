...
void FiberSemaphore::notify() {
    ...
    if(!m_waiters.empty()) {
        ...
        m_waiters.pop_front();
        next.first->scheduler(next.second);
    } else {
        ++m_concurrency;
    }
}
	 ...
	 ,m_name(name)
{
	...
	if(rt) {
		SYLAR_LOG_ERROR(SYLAR_LOG_NAME("system")) << "pthread_create thread failed rt = " << rt 
												  << " name = " << m_name;
...
void Thread::join(){
	if(m_thread) {
		...
		if(rt) {
			SYLAR_LOG_ERROR(SYLAR_LOG_NAME("system")) << "pthread_join thread failed rt = " << rt 
													  << " name = " << m_name;
