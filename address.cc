...
IPAddress::ptr IPv4Address::subnetMask(uint32_t prefix_len) {
	sockaddr_in sub_net;
	memset(&sub_net, 0, sizeof(sub_net));
	sub_net.sin_family = AF_INET;
	sub_net.sin_addr.s_addr = ~byteswapOnLittleEndian(CreatMask<uint32_t>(prefix_len));
	return IPv4Address::ptr(new IPv4Address(sub_net));
}
