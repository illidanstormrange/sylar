...
static sylar::ConfigVar<int>::ptr g_tcp_connect_timeout = 
	sylar::Config::Lookup("tcp.connect.timerout", 5000, "tcp connect timeout");
...
unsigned int sleep(unsigned int seconds) {
	...
	iom->addTimer(seconds * 1000, std::bind((void(sylar::Scheduler::*)
				(sylar::Fiber::ptr, int thread))&sylar::IOManager::scheduler
				, iom, fiber, -1));
	sylar::Fiber::YieldToHold();
	return 0;
}
...
int nanosleep(const struct timespec *req, struct timespec* rem) {
	...
	sylar::Fiber::ptr fiber = sylar::Fiber::GetThis();
	sylar::IOManager* iom = sylar::IOManager::GetThis();
	iom->addTimer(timeout_ms * 1000, std::bind((void(sylar::Scheduler::*)
				(sylar::Fiber::ptr, int thread))&sylar::IOManager::scheduler
				, iom, fiber, -1));
	sylar::Fiber::YieldToHold();
	return 0;
...
int socket(int domain, int type, int protocol) {
	...
	sylar::FdMgr::GetInstence()->get(fd, true);
	return fd;
}
...
int close(int fd) {
	...
	if(ctx) {
		...
		sylar::FdMgr::GetInstence()->del(fd);
	}
	return close_f(fd);
}
...
int setsockopt(int sockfd, int level, int optname, const void *optval, socklen_t optlen) {
	...
	if(level == SOL_SOCKET) {
		if(optname == SO_RCVTIMEO || optname == SO_SNDTIMEO) {
			sylar::FdCtx::ptr ctx = sylar::FdMgr::GetInstence()->get(sockfd);
			if(ctx) {
